...
String apn_type = "IP";
String apn_user = "entelpcs";
String apn_password = "entelpcs";

String client_id = "863427043678730"; // IMEI DEL MODULO SIM (IDENTIFICADOR UNICO POR GPS) 863427043680207
String gps_raw;
String latitude = "";
String longitude = "";
String s_n = "";
...
String readyResponse[] = {"+CGNSSPWR: READY!"};
// ######################################################################
...
void setup() {
  ...
  const int delayInSeconds = 15; // 15 sec

  // esperamos 1 minuto para que inicie bien el modulo.
  for (int i = delayInSeconds; i > 0; --i) {
      lcd_message("Espera " + String(i) + " segundos para que inicie el modulo.");
  ...
  sendATAndWait("AT+CGNSSPWR=1,1", readyResponse, 1, 5, 2);
  sendATAndWait("AT+CGDCONT=1,\"" + apn_type + "\",\"" + apn + "\"", okResponses, 1, 1, 2);
  ...
  sendATAndWait("AT+CMQTTSTART", cmqttstartResponses, 2, 1, 2); 
  sendATAndWait("AT+CMQTTACCQ=0,\"" + String(client_id) + "\",1", cmqttaccqResponses, 2, 1, 2);
  sendATAndWait("AT+CMQTTSSLCFG=0,0", okResponses, 1, 1, 2);
  ...
  enviarMsg("GPS_" + String(client_id) + "/action");
  sendATAndWait("AT+CMQTTSUB=0", cmqttsubResponses, 2, 1, 2);
  sendATAndWait("AT+CGPSINFO", okResponses, 1, 10, 80);
